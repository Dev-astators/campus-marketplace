...
export default function Navbar({ user }) {
  return (
    <header className="w-full bg-gray-100 px-6 py-3 flex items-center gap-6 border-b border-gray-200">
      {/* Brand */}
      <a href="/" className="text-2xl font-extrabold text-blue-700 tracking-tight shrink-0" style={{ fontFamily: 'Inter, sans-serif' }}>
        UniSquare
      </a>
      ...
      {/* User info */}
      <nav aria-label="User menu" className="flex items-center gap-3 ml-auto shrink-0">
        <span className="text-sm font-medium text-gray-800">{user?.name ?? 'Student'}</span>
        <figure className="w-9 h-9 rounded-full bg-gray-300 overflow-hidden flex items-center justify-center">
          {user?.avatarUrl ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-gray-500" aria-hidden="true">
              <path fillRule="evenodd" d="M12 12a5 5 0 1 0 0-10 5 5 0 0 0 0 10Zm-7 8a7 7 0 0 1 14 0H5Z" clipRule="evenodd" />
            </svg>
          )}
          <figcaption className="sr-only">{user?.name ?? 'Student'} profile picture</figcaption>
        </figure>
      </nav>
    </header>
  );
